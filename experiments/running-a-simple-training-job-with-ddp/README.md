...
```bash
flexai training run fcs-experiments-simple-ddp --source-name fcs-experiments --dataset gpt2-tokenized-wikitext \
  --nodes 2 --accels 8 \
...
```bash
flexai training fetch fcs-experiments-simple-ddp
```
...
_Unzip_ the `output_0.zip` file you downloaded [earlier](#step-5-fetching-the-trained-model-artifacts) when running the `flexai training fetch fcs-experiments-simple-ddp` command. This will create a directory named `output` and make note of its location, you will use it next.
