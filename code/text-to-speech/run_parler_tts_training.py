...
# Adapted from https://github.com/huggingface/parler-tts/blob/5d0aca9753ab74ded179732f5bd797f7a8c6f8ee/training/run_parler_tts_training.py
...
from training.eval import clap_similarity, si_sdr, wer
...
def main():
    ...
    parser = HfArgumentParser(
        (ModelArguments, DataTrainingArguments, ParlerTTSTrainingArguments)
    )
    ...
    accelerator.init_trackers(
        ...
        init_kwargs=(
            {"wandb": {"name": data_args.wandb_run_name}}
    ...
    prompt_tokenizer = AutoTokenizer.from_pretrained(
        ...
        or model_args.description_tokenizer_name
    ...
    if model_args.use_fast_tokenizer:
        ...
        description_tokenizer.deprecation_warnings["Asking-to-pad-a-fast-tokenizer"] = (
            True
        )
    ...
    if data_args.save_to_disk and len(os.listdir(data_args.save_to_disk)) > 0:
        dataset_was_precomputed = True
        logger.info(f"dataset_was_precomputed: True")
        logger.info(f"Loading dataset from disk: {data_args.save_to_disk}")
        ...
        logger.info(f"dataset_was_precomputed: False")
        ...
        if data_args.description_column_name is not None:
            columns_to_keep["description_column_name"] = (
                data_args.description_column_name
            )
    ...
    config.decoder.update(
        {
            "cross_attention_implementation_strategy": (
                model_args.cross_attention_implementation_strategy
    ...
    if not dataset_was_precomputed:
        ...
        if (
            description_column_name is not None
            and data_args.max_text_length is not None
        ):
            with accelerator.local_main_process_first():
        ...
        if training_args.torch_compile:
            audio_decoder = accelerator.prepare_model(
                model.audio_encoder, evaluation_mode=True
            )
        else:
        ...
        def apply_audio_decoder(batch):
            ...
            with torch.no_grad():
                labels = audio_decoder.encode(**batch, bandwidth=bandwidth)[
                    "audio_codes"
                ]
            output = {}
            ...
            max_length = (
                len_audio.max() if padding != "max_length" else max_target_length
            )
        ...
        def postprocess_dataset(labels):
            ...
            #  - [B, B, B, B, g, h, E]
            labels = torch.where(
                delay_pattern_mask == -1, audio_encoder_eos_token_id, delay_pattern_mask
            )
        ...
        for split in vectorized_datasets:
            ...
            start_step = get_last_codec_checkpoint_step(
            ...
            if start_step < total_inference_steps:
                for i, batch in enumerate(
                    ...
                    generate_labels = accelerator.pad_across_processes(
                        generate_labels, dim=1, pad_index=0
                    )
                    ...
                    if accelerator.is_main_process:
                        ...
                        lab = [
                            l[:, : int(ratio * length)]
                            for (l, ratio, length) in zip(lab, rat, lens)
                        ]
                        ...
                        if ((cur_step + 1) % data_args.save_codec_steps == 0) or (
                            ...
                            tmp_labels = Dataset.from_dict(
                                {
                                    "labels": all_generated_labels,
                                    "target_length": all_lens,
                                }
                            )
            ...
            with accelerator.local_main_process_first():
                tmp_labels = load_all_codec_checkpoints(
                    os.path.join(data_args.temporary_save_to_disk, split)
                ).select(range(len(vectorized_datasets[split])))
                logger.info(
                    f"Concatenating {split}: {tmp_labels} with {vectorized_datasets[split]}"
                )
                vectorized_datasets[split] = concatenate_datasets(
                    [vectorized_datasets[split], tmp_labels], axis=1
                )
        ...
        if (
            description_column_name is not None
            and data_args.max_description_token_length is not None
        ):
            with accelerator.local_main_process_first():
                # filter description that is shorter than max_text_length
                vectorized_datasets = vectorized_datasets.filter(
    ...
    if data_args.preprocessing_only and data_args.save_to_disk is None:
        ...
        logger.info(
            f"Data preprocessing finished. Files save at {data_args.save_to_disk}"
        )
        return
    ...
    if accelerator.is_main_process:
        ...
        if (
            ...
            or (
                model_args.prompt_tokenizer_name
                == model_args.description_tokenizer_name
            )
        ):
        ...
        if mixed_precision == "fp16":
            ...
            with accelerator.autocast(autocast_handler=autocast_kwargs):
                ...
                if batch.get("attention_mask", None) is not None:
                    encoder_hidden_states = (
                        encoder_hidden_states
                        * batch.get("attention_mask", None)[..., None]
                    )
        ...
        for batch in train_dataloader:
            ...
            if accelerator.sync_gradients:
                ...
                if training_args.do_eval and (
                    ...
                    eval_metrics = {
                        key: torch.mean(torch.cat([d[key] for d in eval_metrics])).to(
                            "cpu"
                        )
                    ...
                    if training_args.predict_with_generate:
                        generated_audios, input_ids, prompts = release_memory(
                            generated_audios, input_ids, prompts
                        )
